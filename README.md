...
```kotlin
// Forward — push a key
backStack.add(AppKey.ProductDetail(ProductDetailArgs(product, ProductSource.LIST)))
...
// Destination with a typed args wrapper — avoids boolean flags
@Serializable
...
> All types embedded in a key must also be `@Serializable`. In this project `Product`, `ProductDetailArgs`, and `ProductSource` are all annotated.

---

### 2.3 Args wrapper — avoid boolean flags in keys

Putting a raw `Boolean` in a key is a smell: it mixes navigation identity with UI behavior, and scales badly as entry points grow.

**Don't:**
```kotlin
data class ProductDetail(val product: Product, val fromHistory: Boolean)
```

**Do — wrap in a typed args class:**
```kotlin
@Serializable
...
@Serializable
enum class ProductSource { LIST, HISTORY }

// Key stays clean
data class ProductDetail(val args: ProductDetailArgs) : AppKey()
```

Adding a new entry point (e.g. push notification) is one enum value — no changes to `AppKey`:
```kotlin
enum class ProductSource { LIST, HISTORY, NOTIFICATION }
```

The screen uses `args.source` to decide behavior:
```kotlin
...
### 2.4 Passing data through the key — fallback-first strategy

Because the full `Product` object lives in the nav key, the detail screen always has data **before** any network call completes. This implements the pattern your team uses for network errors:

```
Navigate to detail
      │
      ▼
ViewModel init: set state = cachedProduct (from key) immediately → no blank screen
      │
      ▼
fetchFreshData() runs in background
      ├── ✅ success → replace state with fresh data, isFreshData = true
      └── ❌ failure → keep cached data, show error banner + Retry button
```

```kotlin
class ProductDetailViewModel(private val args: ProductDetailArgs, ...) : ViewModel() {

    private val _state = MutableStateFlow(
        // Cached product from nav key — shown instantly
        State(product = args.product, isRefreshing = true)
    )
    ...
    private fun fetchFreshData() {
        viewModelScope.launch {
            try {
                ...
                // Network failed — cached data is already visible
                _state.update { it.copy(isRefreshing = false, error = "Showing cached data.") }
            }
        }
    }
}
...
To simulate a network error, uncomment one line in `ProductRepository`:
```kotlin
suspend fun fetchProductById(id: Int): Product {
    delay(800)
    throw Exception("Network unavailable") // ← uncomment to test fallback
    ...
}
```
...
**DSL style** (used in this project — built-in fallback error if key is missing):
```kotlin
entryProvider = entryProvider {
    ...
    entry<AppKey.History> {
        HistoryScreen(
            onNavigateToDetail = { product ->
                backStack.add(AppKey.ProductDetail(ProductDetailArgs(product, ProductSource.HISTORY)))
            }
        )
    }
    entry<AppKey.ProductDetail> { key ->
        ProductDetailScreen(args = key.args, ...)
    }
}
```

`entry<T>` uses reified generics to match by type. The lambda receives the key as `it` (or a named param).
...
// Inside an entry, use navEntryViewModel() instead of viewModel()
entry<AppKey.ProductDetail> { key ->
    val vm: ProductDetailViewModel = navEntryViewModel(
        factory = ProductDetailViewModel.Factory(key.args)
    )
    ProductDetailScreen(viewModel = vm)
}
```

This project uses `viewModel()` (activity-scoped) with a unique `key` string per product — sufficient for a demo but switch to `navEntryViewModel()` for production to avoid ViewModel leaks.
...
User taps a product
      │
      ▼
ProductListViewModel emits Effect.NavigateToDetail(product)
      │
      ▼
ProductListScreen collects effect → calls onNavigateToDetail(product)
      │
      ▼
MainActivity: backStack.add(AppKey.ProductDetail(ProductDetailArgs(product, LIST)))
      │
      ▼
NavDisplay recomposes → entry<AppKey.ProductDetail> runs
      │
      ▼
ProductDetailScreen(args) rendered — cached product shown immediately
      │
      ▼
ProductDetailViewModel.fetchFreshData() runs in background
      ├── ✅ fresh data → UI updates silently
      └── ❌ network error → cache banner shown, Retry available
```

---

### 2.9 Screen navigation map

```
ProductList ──[tap product]──────────────► ProductDetail (source = LIST)
     │                                           │
     │                                      [Add to Cart]
     │                                           │
     └──[History icon]──► History ◄─────────────┘
                         ...
                         [tap order]
                              │
                              ▼
                        ProductDetail (source = HISTORY)
                        (no Add to Cart button)
```

---

### 2.10 Nav 2 vs Nav 3 cheat sheet

| | Nav 2 | Nav 3 |
|---|---|---|
| Navigate | `navController.navigate("detail/{id}")` | `backStack.add(DetailKey(args))` |
| Back | `navController.popBackStack()` | `backStack.removeLastOrNull()` |
| Arguments | Route strings / `savedStateHandle` | Fields on the key / args wrapper |
| Pass cached data | Not possible | Embed full model in key |
| Back stack ownership | Library | **You** |
| ViewModel scoping | Automatic via `hiltViewModel` | Opt-in via `viewModelStoreNavEntryDecorator` |
| Adaptive layouts | Manual workaround | Built-in `SceneStrategy` |
| State survival | `@Serializable` routes | `@Serializable` keys + embedded models |
